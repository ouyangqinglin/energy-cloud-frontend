  ...
  const rightItems = (
    <>
      <div className="card-temp-item temp-right-item">
        ...
        <div>
          {formatMessage({ id: 'siteMonitor.temperature', defaultMessage: '温度' })}{(currentPage - 1) * 13 + 6}:
  ...
  items.forEach((_, index) => {
    upItems.push(
      <div className="card-battery-item" key={'up' + index}>
        <span>
        {formatMessage({ id: 'siteMonitor.cell', defaultMessage: '电芯' })}1_{currentPage}_{(currentPage - 1) * 24 + index + 1}
        </span>
    ...
    downItems.push(
      <div className="card-battery-item" key={'down' + index}>
        <span>
        {formatMessage({ id: 'siteMonitor.cell', defaultMessage: '电芯' })}1_{currentPage}_{(currentPage - 1) * 24 + index + 13}
        </span>
        <div className="card-battery-value">
  ...
  tempArr.forEach((_, index) => {
    upTempItems.push(
      <div className="card-temp-item" key={'uptemp' + index}>
        <span className="card-temp-dot" />
        <div>
        {formatMessage({ id: 'siteMonitor.temperature', defaultMessage: '温度' })}{(currentPage - 1) * 13 + index + 1}:
          <span className="card-temp-value">
    ...
    downTempItems.push(
      <div className="card-temp-item" key={'downtemp' + index}>
        <div>
        {formatMessage({ id: 'siteMonitor.temperature', defaultMessage: '温度' })}{(currentPage - 1) * 13 + index + 7}:
          <span className="card-temp-value">
  ...
  const runItems = [
    {
      ...
      children: (
        <>
          ...
          <Label title={formatMessage({ id: 'siteManage.set.statusInformation', defaultMessage: '状态信息' })} />
          <Detail data={emsData || {}} items={statusItems} column={4} />
          <Label title={formatMessage({ id: 'device.setInformation', defaultMessage: '设置信息' })} />
    ...
    },
    {
      ...
      children: (
        <>
          ...
          <Label title={formatMessage({ id: 'device.measurementInformation', defaultMessage: '测量信息' })} />
    ...
    },
    {
      ...
      children: (
        <>
          ...
          <Label title={`${formatMessage({ id: 'device.module', defaultMessage: '模组' })}（${currentPage}）${formatMessage({ id: 'siteMonitor.monomerInfo', defaultMessage: '单体信息' })}`} />
