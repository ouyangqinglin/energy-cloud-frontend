...
export type ControlType = {
  emsSysStatus: boolean;
  MainContactorStatus: boolean;
  sysModel: boolean;
  sysReset: boolean;
};
...
const timeMap = new Map([
  ['time1', 'pcsRunningTimeFrame1'],
  ['time2', 'pcsRunningTimeFrame2'],
  ['time3', 'pcsRunningTimeFrame3'],
  ['time4', 'pcsRunningTimeFrame4'],
  ['time5', 'pcsRunningTimeFrame5'],
]);
...
const Setting: React.FC<SettingProps> = (props) => {
  ...
  const onControlChange = useCallback(
    (fieldValue: ControlType) => {
      const field: keyof ControlType = Object.keys(fieldValue)[0] as keyof ControlType;
          ...
          return run({
            ...
            .then((data) => {
              if (data) {
                ...
                controlForm.setFieldValue(field, !fieldValue[field]);
              }
            })
            .catch(() => {
              controlForm.setFieldValue(field, !fieldValue[field]);
        ...
        onOk: () => {
          ...
          for (const key in formData) {
            if (key.includes('power')) {
              ...
              field = timeMap.get(key) || '';
              inputData[field] =
                formData[key][0].format('HH:mm') + '-' + formData[key][1].format('HH:mm');
            }
          }
          run({
            deviceId: id,
    ...
    if (!lodash.isEmpty(settingData)) {
      if (disableRun) {
        ...
        timeMap.forEach((item, key) => {
          const value = settingData?.[item]?.split('-') || [];
          runData[key] = value.length
            ? [moment(`2023-06-07 ${value[0]}`), moment(`2023-06-07 ${value[1]}`)]
            : [];
        });
      ...
      controlForm.setFieldsValue({
        emsSysStatus: settingData?.emsSysStatus,
        MainContactorStatus: settingData?.MainContactorStatus,
        sysModel: settingData?.sysModel,
      });
    }
  }, [settingData, runForm, timeForm]);
