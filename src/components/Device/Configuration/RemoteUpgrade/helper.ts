...
export const upgradeItems: DetailItem[] = [
  { label: formatMessage({ id: 'device.currentVersion' , defaultMessage: '当前版本' }), field: 'nowVersion' },
  {
    ...
    format: (value: UpgradeDataType[]) => {
      return value
        ?.map?.((item) => {
          return item.version + (item?.status === 0 ? `(${formatMessage({ id: 'common.disable' , defaultMessage: '禁用' })})` : '');
        })
        ?.join('，');
    },
  },
