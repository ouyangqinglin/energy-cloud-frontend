...
const bingData = (data: MonitorDataType[], type: string, index: number) => {
  if (!data.length) {
    data.push({
      ...
      deviceName: index ? formatMessage({ id: 'siteManage.set.associateDataCollectionPoints', defaultMessage: '关联数据采集点' }) : formatMessage({ id: 'siteManage.set.associateDevice', defaultMessage: '关联设备' }),
...
const Monitor: React.FC = () => {
  ...
  const onSwitchChange = useCallback((type: string, checked: boolean) => {
    if (checked) {
      ...
      Modal.confirm({
        title: formatMessage({ id: 'siteManage.set.closeMonitorSiteConfirm', defaultMessage: '关闭监测点确认' }),
        content: formatMessage({ id: 'siteManage.set.closeMonitorSiteConfirmContent', defaultMessage: '是否关闭监测点，关闭后主监控页面将不在监测该回路' }),
    ...
    (selectedData) => {
      const rowData: MonitorDataType[] = selectedData.map((item: any) => {
        return {
          ...
          collection: selectedRow.area === 'elec' ? formatMessage({ id: 'siteManage.set.allDataCollectionPoints', defaultMessage: '全部数据采集点' }) : item?.node?.paramName,
            ...
            const rowData =
              data?.[item.type]?.valueMap?.[row.subType]?.map?.((record: any) => {
                return {
                  ...
                  collection: row.area == 'elec' ? formatMessage({ id: 'siteManage.set.allDataCollectionPoints', defaultMessage: '全部数据采集点' }) : record?.paramName,
  ...
  const panelData = [
    { key: 'electric', title: formatMessage({ id: 'siteManage.set.mainsMonitor', defaultMessage: '市电监测' }) },
    { key: 'load', title: formatMessage({ id: 'siteManage.set.otherLoad', defaultMessage: '其他负载' }) },
    { key: 'photovoltaic', title: formatMessage({ id: 'siteManage.set.pvMonitor', defaultMessage: '光伏监测' }) },
    { key: 'energy', title: formatMessage({ id: 'siteManage.set.energyStorageMonitor', defaultMessage: '储能监测' }) },
    { key: 'charge', title: formatMessage({ id: 'siteManage.set.chargePileMonitor', defaultMessage: '充电桩监测' }) },
  ];
  ...
  return (
    <>
      ...
      <TableTreeModal
        ...
        title={selectedRow.area === 'elec' ? formatMessage({ id: 'common.selectDevice', defaultMessage: '选择设备' }) : formatMessage({ id: 'common.selectDataCollectionPoints', defaultMessage: '选择数据采集点' })}
