export default {
  // 只放一些业务无关的不会变化的基础单词和单位

  'common.new': '新建',
  'common.add': '新增',
  'common.delete': '删除',
  'common.edit': '编辑',
  'common.refresh': '刷新',
  'common.detail': '查看详情',
  'common.view': '详情',
  'common.productType': '产品类型',
  'common.model': '产品型号',
  'common.softwarePackage': '软件包名',
  'common.software': '软件包',
  'common.updataSoftware': '请上传软件包',
  'common.uploadDate': '上传时间',
  'common.index': '序号',
  'common.sort': '排序',
  'common.versionSentence': '{name}版本',
  'common.version': '版本号',
  'common.description': '描述',
  'common.uploader': '上传人',
  'common.status': '状态',
  'common.operate': '操作',
  'common.operator': '操作人',
  'common.currentState': '当前状态',
  'common.current': '当前',
  'common.newBuilt': '新建',
  'common.module': '模块',
  'common.selectDevice': '选择设备',
  'common.selectDataCollectionPoints': '选择数据采集点',
  'common.disable': '禁用',
  'common.enable': '启用',
  'common.enabled': '使能',
  'common.effect': '生效',
  'common.ineffect': '未生效',
  'common.set': '设置',
  'common.setDetails': '设置详情',
  'common.deviceName': '设备名称',
  'common.deviceSn': '设备序列号',
  'common.deviceCode': '设备ID',
  'common.equipmentSerial': '设备序列号',
  'common.equipmentEmsSerial': 'ems序列号',
  'common.currentVersion': '当前版本',
  'common.original': '原始版本',
  'common.siteName': '站点名称',
  'common.upload': '上传',
  'common.supportFile': '支持扩展名',
  'common.ok': '确定',
  'common.confirm': '确认',
  'common.cancel': '取消',
  'common.hide': '隐藏',
  'common.displayRealTimeInfo': '显示实时信息',
  'common.viewDetail': '查看详情',
  'common.unbind': '解绑',
  'common.unbindSuccess': '解绑成功',
  'common.custom': '自定义',
  'common.predefine': '预定义',
  'common.Topping': '置顶',

  'common.operateSuccess': '操作成功',
  'common.refreshSuccess': '刷新成功',
  'common.power': '功率',
  'common.groupString': '组串',
  'common.dayPowerConsumption': '日充电量',
  'common.totalPowerConsumption': '累计充电量',
  //

  'common.screen': '大屏页',
  'common.screenArchitecture': '大屏架构图',
  'common.alarmDisplayConfiguration': '告警显示配置',
  'common.time.day': '日',
  'common.time.day1': '天',
  'common.time.month': '月',
  'common.time.oneMonth': '一个月',
  'common.time.threeMonth': '三个月',
  'common.time.sixMonth': '六个月',
  'common.time.oneYear': '一年',
  'common.time.year': '年',
  'common.time.total': '累计',
  'common.unit': '单位',
  'common.area': '地区',
  'common.country': '国家',
  'common.province': '省份',
  'common.city': '城市',
  'common.lastOperationTime': '最后操作时间',
  'common.operationTime': '操作时间',
  'common.confirmDelete': '请确认是否删除',
  'common.baseInfo': '基础信息',
  'common.pleaseEnter': '请输入',
  'common.remark': '备注',
  'common.uploadPicture': '上传图片',
  'common.address': '地址',
  'common.coordinate': '坐标',
  //
  'common.not': '无',
  'common.all': '全部',
  'common.del': '删除成功',
  'common.addSuccess': '新增成功',
  'common.editSuccess': '修改成功',
  'common.createTime': '创建时间',
  'common.deliveryTime': '投运日期',
  'common.lastPer': '最后更新人',
  'common.tobeDone': '待执行',
  'common.upgradeDuring': '升级中',
  'common.upgradeSuc': '升级成功',
  'common.upgradeFai': '升级失败',
  'common.execute': '执行',
  'common.site.siteName': '站点名称',
  'common.site.siteType': '站点类型',
  'common.workbentch': '工作台',
  'common.export': '导出',
  'common.import': '导入',
  'common.addTime': '创建时间',
  'common.upTime': '上线时间',
  'common.unknown': '未知',
  'common.other': '其他',
  'common.search': '搜索',
  'common.successSaved': '保存成功',
  'common.save': '保存',
  'common.saveAndeffect': '保存并生效',
  'common.uploadPhoto': '上传图片',
  'common.select': '选择',
  'common.pleaseSelect': '请选择',
  'common.uploadFile': '请上传文件',
  'common.deleteConfirm': '删除确认',
  'common.enter': '进入',
  'common.appointTime': '预约时间',
  'common.recepTime': '接收时间',
  'common.finishTime': '完成时间',
  'common.lastTime': '最后更新时间',
  'common.updatedTime': '更新时间',
  'common.createPerson': '创建人',
  'common.deliveryPerson': '投运人',
  'common.renew': '更新人',
  'common.telephone': '联系电话',
  'common.mailbox': '邮箱',
  'common.done': '已执行',
  'common.userName': '用户名',
  'common.account': '账号',
  'common.password': '密码',
  'common.pleaseEnterDevice': '请输入设备名称',
  'common.pleaseEnterSn': '请输入设备序列号',
  'common.normal': '正常',
  'common.abnormal': '异常',
  'common.warning': '告警',
  'common.historyData': '历史数据',
  'common.shortMessage': '短信',
  'common.notInform': '不通知',
  'common.email': '邮件',
  'common.SMSAndEmail': '短信+邮件',
  'common.notificationMode': '通知方式',
  'common.selectNotificationMode': '请选择通知方式',
  'common.notificationPerson': '通知人',
  'common.selectNotificationPerson': '请选择通知人',
  'common.notConfigured': '未配置',
  'common.stewing': '静置',
  'common.onLine': '在线',
  'common.offline': '离线',
  'common.rmb': '元',
  'common.more': '更多',
  'common.required': '必填',
  'common.start': '开始',
  'common.end': '结束',
  'common.selectDate': '选择日期',
  'common.occurrenceTime': '发生时间',
  'common.time': '时间',
  'common.operatingUser': '操作用户',
  'common.userType': '用户类型',
  'common.logs': '日志',
  'common.configured': '配置',
  'common.configurationInformation': '配置信息',
  'common.modify': '修改',
  'common.systemTime': '系统时间',
  'common.noData': '暂无数据',
  'common.directionalIcon': '方向性图标',
  'common.suggestiveIcons': '提示建议性图标',
  'common.editClassIcon': '编辑类图标',
  'common.dataClassIcon': '数据类图标',
  'common.brandAndLogo': '品牌和标识',
  'common.websiteCommonIcons': '网站通用图标',
  'common.yotaiIcon': '永泰图标',
  'common.closeCurrentLabel': '关闭当前标签',
  'common.closeOtherTags': '关闭其他标签',
  'common.closeAllTags': '关闭全部标签',
  'common.closeCurrentLeftLabel': '关闭当前左边标签',
  'common.closeCurrentrightLabel': '关闭当前右边标签',
  'common.logContent': '日志内容',
  'common.logCode': '日志编码',
  'common.cleared': '清空了',
  'common.clear': '清空',
  'common.reset': '重置',
  'common.viewMore': '查看更多',
  'common.notice': '通知',
  'common.viewedAllNotifications': '你已查看所有通知',
  'common.message': '消息',
  'common.readAllMessages': '您已读完所有消息',
  'common.backlog': '待办',
  'common.completedAllBacklog': '你已完成所有待办',
  'common.activate': '启动',
  'common.stop': '停止',
  'common.should24Hour': '应满24小时',
  'common.timeStartShouldGreaterEnd': '时段{start}开始时间应大于等于时段{end}结束时间',
  'common.timeEndShouldLessStart': '`时段${end}结束时间应小于等于时段${start}开始时间',
  'common.master': '主',
  'common.slave': '从',
  'common.back': '返回',
  'common.eliminated': '已消除',
  'common.alarming': '告警中',
  'common.maxSelectItem': '最多选择{num}项',
  'common.download': '下载',
  'common.periodTime': '时段',
  'common.pleaseEnterSentence': '请输入{content}',
  'common.addSentence': '新增{content}',
  'common.pleaseSelectSentence': '请选择{content}',
  'common.configParam': '配置参数',
  'common.ipAddress': 'IP地址',
  'common.connect': '连接',
  'common.break': '断开',
  'common.serialSentence': '{name}序列号',
  'common.manufacturerSentence': '{name}厂商',
  'common.modelSentence': '{name}型号',
  'common.pause': '暂停',
  'common.roll': '滚动',
  'common.modifyBasicInformation': '修改基本信息',
  'common.amap': '高德',
  'common.google': '谷歌',
  'common.binder': '绑定人',
  'common.binderTime': '绑定时间',
  'common.noneYet': '暂无',
  'common.pageDevelopmenting': '页面开发中，敬请期待',
  'common.next': '下一步',
  'common.minute': '分钟',
  'common.tenThousand': '万',
  'common.tenMillion': '千万',

  'common.requestError10000': '系统未知错误，请反馈给管理员',
  'common.requestError200': '服务器成功返回请求的数据。',
  'common.requestError201': '新建或修改数据成功。',
  'common.requestError202': '一个请求已经进入后台排队（异步任务）。',
  'common.requestError204': '删除数据成功。',
  'common.requestError400': '发出的请求有错误，服务器没有进行新建或修改数据的操作。',
  'common.requestError401': '用户没有权限（令牌、用户名、密码错误）。',
  'common.requestError403': '用户得到授权，但是访问是被禁止的。',
  'common.requestError404': '发出的请求针对的是不存在的记录，服务器没有进行操作。',
  'common.requestError406': '请求的格式不可得。',
  'common.requestError410': '请求的资源被永久删除，且不会再得到的。',
  'common.requestError422': '当创建一个对象时，发生一个验证错误。',
  'common.requestError500': '服务器发生错误，请检查服务器。',
  'common.requestError502': '网关错误。',
  'common.requestError503': '服务不可用，服务器暂时过载或维护。',
  'common.requestError504': '网关超时。',
  'common.requestErrorAbnormalTips': '您的网络发生异常，无法连接服务器',
  'common.requestErrorAbnormal': '网络异常',
  'common.requestError': '请求错误',
  'common.passwordResetSuccessful': '密码重置成功',
  'common.passwordinconsistency': '两次密码输入不一致',

  'common.yes': '是',
  'common.no': '否',
  'common.1001': '直接跳转',
  'common.1002': 'SSO跳转',
  'common.1003': 'Url格式错误',
  'common.1004': '请配置',
  'common.1005': '令牌获取失败，请重试！',
  'common.1006': '次',
  'common.1007': '不支持拖拽到子节点',
  'common.1008': '不支持置顶子节点',
};
