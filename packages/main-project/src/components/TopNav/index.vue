      ...
      if (router.children[item as any].parentPath === undefined) {
        if (router.path === '/') {
          ...
          if (!isHttp(router.children[item as any].path)) {
            router.children[item as any].path = `${router.path}/${
              router.children[item as any].path
            }`
          }
        }
...
const activeMenu = computed(() => {
  ...
  if (
    path !== undefined &&
    path.lastIndexOf('/') > 0 &&
    !hideList.includes(path)
  ) {
    const tmpPath = path.substring(1, path.length)
    activePath = `/${tmpPath.slice(0, Math.max(0, tmpPath.indexOf('/')))}`
    if (!route.meta.link) {
...
function setVisibleNumber() {
  ...
  visibleNumber.value = Number.parseInt(String(width / 85))
}
